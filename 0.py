...
def prompt_weather(circuit:str, session:str):
    ...
    usr_p = (
      f"Circuit: {circuit}\nSession: {session}\n"
      "Give realistic weather considering regional climate and seasonality. Some circuits rarely see rain.\n"
      "Return fields: air_temp_c, track_temp_c, rain_prob (0-1), rain_intensity (0-1), wetness (0-1), grip_base (0.6-1.05).\n"
      "Wetness should roughly be rain_intensity * rain_prob but can lag (drying track)."
    )
    return sys_p, usr_p
...
def prompt_ai_plan(session:str, circuit:str, duration:int, drivers_non_player:list[dict], lap_base:float):
    sys_p = "You are an F1 race engineer. Output ONLY JSON that follows the provided JSON schema."
    lines = "\n".join([f"- {d['name']} ({d['team']})" for d in drivers_non_player])
    usr_p = (
      f"Session: {session}\nCircuit: {circuit}\nDuration(sec): {duration}\n"
      f"Ref lap(sec): {lap_base:.3f}\nDrivers (exclude player's team):\n{lines}\n\n"
      "Rules:\n"
      "- base_vmul in [0.96, 1.12]\n"
      "- Each driver makes 1–3 runs. For each run provide {start_sec, laps, timed_laps}.\n"
      "- 'laps' = number of hot laps. After last hot lap, do one in-lap and box via pitIn → pitStop.\n"
      "- Spread starts across the session."
    )
    return sys_p, usr_p

def get_ai_plan(session, circuit, duration_sec, drivers, player_team, lap_base):
    others = [d for d in drivers if d["team"] != player_team]
    try:
        from f1sim.ai.llm_client import ask_llm_json
        sys_p, usr_p = prompt_ai_plan(session, circuit, duration_sec, others, lap_base)
        js = ask_llm_json(AI_PLAN_SCHEMA, sys_p, usr_p, temperature=0.2)
        ok = {(d["name"], d["team"]) for d in others}
        return [p for p in js.get("plans", []) if (p.get("name"), p.get("team")) in ok]
    except Exception:
        rnd = random.Random(123)
        plans=[]
        for d in others:
            rating = float(d["rating"]) if d.get("rating") is not None else 80.0 + rnd.uniform(-5,5)
            base_v = 0.98 + (max(60.0, min(100.0, rating))-60.0)*(0.12/40.0)
            t0 = rnd.uniform(10, 120)
            t1 = t0 + rnd.uniform(300, 520)
            plans.append({
              "name": d["name"], "team": d["team"], "base_vmul": round(base_v,3),
              "runs":[ {"start_sec": t0, "laps":3, "timed_laps":[2]},
                       {"start_sec": min(duration_sec-120, t1), "laps":3, "timed_laps":[2]} ]
            })
        return plans
...
# ── 세션 실행 ─────────────────────────────────────
def run_session(SESSION: str, DURATION_MIN: int):
    st.set_page_config(layout="wide", page_title=f"{SESSION} — Qualifying")

    # URL 쿼리에서 결과가 올라왔으면 바로 저장 + Q2 전환
    qp = st.experimental_get_query_params()
    if "quali_result_b64" in qp:
        try:
            b64 = qp.get("quali_result_b64", [""])[0]
            js = json.loads(base64.b64decode(b64.encode("ascii")).decode("utf-8"))
            outp = persist_quali_result(SESSION, js)
            st.toast(f"{SESSION} 결과 저장 완료: {outp}", icon="✅")
        except Exception as e:
            st.warning(f"결과 저장 실패: {e}")
        # 쿼리 제거(중복 저장 방지)
        st.experimental_set_query_params()
        # 이동 지시가 있으면 페이지 전환 시도
        goto = qp.get("goto", [""])[0].upper()
        if goto in {"Q2","Q3"}:
            for cand in [
                f"pages/05b_{goto.lower()}_svg.py",
                f"pages/{goto.lower()}_svg.py",
                f"pages/quali_{goto.lower()}_svg.py",
            ]:
                try:
                    st.switch_page(cand)
                    return
                    ...
                    pass
            st.info(f"{goto} 페이지로 전환할 수 없습니다. 좌측 사이드바에서 {goto}를 선택하세요.")
    ...
    if not roster:
        st.error("drivers.csv를 찾지 못했거나 비어 있습니다. (info/ 또는 data/)")
        st.stop()
...
<style>
  ...
  /* 오른쪽: 팀 텔레메트리 + 전략 타임라인 */
  .bar { width:100%; height:10px; background:#0f172a; border:1px solid #334155; border-radius:999px; overflow:hidden; }
  .bar > div { height:100%; background:#0b5cff; width:0%; }
  .bar.alt > div { background:#22c55e; }
  .tmCard { border:1px solid #1e293b; border-radius:10px; padding:10px; margin-top:8px; background:#0b1220; }

  /* Strategy Timeline */
  #strategyBox { background:#0b1220; border:1px solid #1e293b; border-radius:12px; padding:12px; margin-top:10px; }
  #strategyHead { display:flex; justify-content:space-between; align-items:center; margin-bottom:6px; }
  #strategySVG { width:100%; height:200px; display:block; background:#0e1626; border:1px solid #203049; border-radius:10px; }
</style>
...
<div id="wrap">
    ...
    <div id="sessionDone">
      <div class="panel">
        ...
        <div style="display:flex; gap:8px; justify-content:center;">
          ...
          <button id="btnNext" class="btn primary">Q2로 넘어가기</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div id="right">
    ...
    <div id="strategyBox">
      <div id="strategyHead">
        <div style="font-weight:700;">Strategy Timeline</div>
        <div class="muted">
          대상:
          <select id="strategySel" class="opt"></select>
        </div>
      </div>
      <svg id="strategySVG" viewBox="0 0 520 200" preserveAspectRatio="none">
        <g id="sgAxes"></g>
        <path id="sgCurve" fill="none" stroke="#60a5fa" stroke-width="2"/>
        <line id="sgCursor" x1="0" y1="8" x2="0" y2="192" stroke="#f43f5e" stroke-width="2"/>
        <g id="sgLabels"></g>
      </svg>
      <div class="muted" style="margin-top:6px;">x: 랩 진행도 · y: 타이어 잔량(%) — 페이스/노면/컴파운드 반영 예측</div>
    </div>
  </div>
</div>
...
(function(){
  ...
  // ── 물리 파라미터 ──────────────────────────────
  const TRANS_SIM = 0.25;                      // 경로 전환 시뮬 시간(s)
  const PIT_WAIT_SEC = 4.0;                    // 피트스톱 정차 시간
  const FUEL_PER_LAP = 1.6;                    // 기준 랩당 연료(kg)
  const FUEL_FLOW_MAX = 100.0/3600.0;          // FIA 100 kg/h → kg/s
  ...
  const selTeamSmall  = document.getElementById('selTeamSmall');
  const selMode  = document.getElementById('selMode');
  const selTy    = document.getElementById('selTy');
  const selBest  = document.getElementById('selBest');
  const selGap   = document.getElementById('selGap');
  const selProg  = document.getElementById('selProg');
  const selLaps  = document.getElementById('selLaps');

  document.querySelectorAll('.speed .btn').forEach(b=>{
    b.addEventListener('click', ()=>{
      const m = parseFloat(b.getAttribute('data-m')||'1');
      if (Number.isFinite(m)) SPEED = m;
    });
  });
  ...
  // 피트스톱 정차 중인지(교체 가능 상태)
  function canChangeTireNow(car){
    return !!car && car.mode === 'pitStopWait';
  }
  ...
  window.releaseNow = function(name, team, laps=3, timed=[2]){
    ...
    const lapsFuel = 1 + car.lapsLeft + 1;   // out + hot laps + in
  ...
  // 하단 도크: 우리팀 카드
  const playerCtrlRefs = {}; // key: name|team → {selTy, paceSel, fuelSel, tyIcon}
  ...
  function playerCards(){
    ...
    players.forEach((p)=>{
      ...
      const applyToCar = (forceIcon=false)=>{ 
        ...
        car.pace = paceSel.value; 
        car.fuelMix = fuelSel.value; 
  ...
  function refreshPlayerCardStates(){
    PLAN.filter(p=>p.isPlayer).slice(0,2).forEach(p=>{
      const refs = playerCtrlRefs[keyOf(p)]; if(!refs) return;
      const car = findCar(p.name,p.team); if(!car) return;
  ...
  function updateTeamTelemetry(){
    PLAN.filter(p=>p.isPlayer).slice(0,2).forEach((p,ix)=>{
      ...
      const stat = document.getElementById(`tm${ix}Stat`);
      if (stat) stat.textContent = `${car.pace} · ${car.fuelMix} · Lap ${car.lap}`;
  ...
  function buildStrategyUI(){
    const sel = document.getElementById('strategySel'); if(!sel) return;
    sel.replaceChildren();
    PLAN.filter(p=>p.isPlayer).slice(0,2).forEach((p,i)=>{
      const o=document.createElement('option'); o.value=String(i); o.textContent = `#${p.abbr} ${p.name}`; sel.appendChild(o);
    });
    if (sel.children.length>0) sel.value = "0";
    const axes = document.getElementById('sgAxes');
    axes.innerHTML = '';
    const mkText=(x,y,t)=>{ const el=document.createElementNS(stage.namespaceURI,'text'); el.setAttribute('x',x); el.setAttribute('y',y); el.setAttribute('font-size','10'); el.setAttribute('fill','#cbd5e1'); el.textContent=t; return el; };
    axes.appendChild(mkText(4,12,'100%'));
    axes.appendChild(mkText(4,100,'50%'));
    axes.appendChild(mkText(4,196,'0%'));
  }
  function predictWear(car, lapsHorizon=6){
    ...
    const pts=[]; let life=car.tireLife;
    const steps = Math.max(60, lapsHorizon*60);
    for(let i=0;i<=steps;i++){
      const xLaps = (i/steps)*lapsHorizon;
      const lifeEst = Math.max(0, 1 - (1-life) - wearLap * xLaps);
      pts.push([xLaps, lifeEst]);
    }
    return pts;
  }
  function drawStrategy(car){
    const svg = document.getElementById('strategySVG'); if(!svg||!car) return;
    const W = 520, H = 200, LAPS_H = 6;
    const pts = predictWear(car, LAPS_H);
    const sx = x => 40 + (W-48) * (x / LAPS_H);
    const sy = y => 8 + (H-16) * (1 - y);
    const d = pts.map((p,i)=> (i? 'L':'M') + sx(p[0]) + ' ' + sy(p[1])).join(' ');
    document.getElementById('sgCurve').setAttribute('d', d);
    const progress = (car.lap + car.s) % LAPS_H;
    const cx = sx(progress);
    const cur = document.getElementById('sgCursor'); cur.setAttribute('x1', cx); cur.setAttribute('x2', cx);
    const labels = document.getElementById('sgLabels'); labels.innerHTML='';
    for(let k=0;k<=LAPS_H;k++){
      const x = sx(k);
      const t = document.createElementNS(stage.namespaceURI,'text'); t.setAttribute('x', x-4); t.setAttribute('y', 196); t.setAttribute('font-size','10'); t.setAttribute('fill','#cbd5e1'); t.textContent=String(k);
      labels.appendChild(t);
    }
  }
  function updateStrategy(){
    const sel = document.getElementById('strategySel'); if(!sel || sel.children.length===0) return;
    const idx = parseInt(sel.value||'0',10);
    const player = PLAN.filter(p=>p.isPlayer).slice(0,2)[idx];
    const car = player ? findCar(player.name, player.team) : null;
    if (car) drawStrategy(car);
  }
  ...
  function start(){
    ...
    if (pendingReleases.length){ pendingReleases.splice(0).forEach(x=>window.releaseNow(x.name, x.team, x.laps, x.timed)); }
    if (pendingBoxes.length){ pendingBoxes.splice(0).forEach(x=>window.boxNow(x.name, x.team)); }
