    ...
    try:
        ...
        WEATHER_SCHEMA = {
          ...
          "schema":{"type":"object","additionalProperties":False,
            "properties":{
              "version":{"type":"string"},"circuit":{"type":"string"},"session":{"type":"string"},
              "air_temp_c":{"type":"number"},"track_temp_c":{"type":"number"},
        ...
        usr_p = (f"Circuit: {circuit}\nSession: {session}\n"
                 "Give realistic weather; don't force unlikely rain.\n"
                 "Return fields: air_temp_c, track_temp_c, rain_prob, rain_intensity, wetness, grip_base.")
        ...
        rain_int  = 0.20 if wet_round else 0.0
        wetness   = min(1.0, rain_prob * rain_int * 1.5)
        return {"version":"0.1","circuit":circuit,"session":session,
...
def get_ai_plan(session, circuit, duration_sec, drivers, player_team, lap_base):
    """
    외부 LLM이 있으면 사용하고, 없거나 응답이 이상하면 항상 안전한 플랜을 생성합니다.
    - 반환 형태: [{"name","team","base_vmul","runs":[{"start_sec","laps","timed_laps"}]}...]
    - constraints: base_vmul ∈ [0.96, 1.12], laps ∈ [1, 6], start_sec ∈ [0, duration_sec-60]
    """
    import random
    ...
    def _mk_default_runs(rnd, dur):
        # 두 번의 러닝을 기본으로 (dur 범위 안)
        t0 = rnd.uniform(10, min(120, max(10, dur*0.15)))
        t1 = t0 + rnd.uniform(240, min(520, max(180, dur*0.55)))
        t1 = clamp(t1, 0, max(0, dur-60))
        return [
            {"start_sec": float(clamp(t0, 0, max(0, dur-60))), "laps": 3, "timed_laps": [2]},
            {"start_sec": float(clamp(t1, 0, max(0, dur-60))), "laps": 3, "timed_laps": [2]},
        ]

    def _coerce_one(plan_obj, dur):
        """LLM/임의 생성 플랜 1건을 스키마에 맞춰 보정"""
        name = _safe_name(plan_obj.get("name"))
        team = _safe_name(plan_obj.get("team"))
        if not name or not team:
            return None

        try:
            base_v = float(plan_obj.get("base_vmul", 1.0))
        except Exception:
            base_v = 1.0
        base_v = float(clamp(base_v, 0.96, 1.12))

        runs_in = plan_obj.get("runs") or []
        runs_out = []
        for r in runs_in:
            try:
                start = float(r.get("start_sec", 0.0))
                laps  = int(r.get("laps", 3))
                start = float(clamp(start, 0.0, max(0.0, float(dur) - 60.0)))
                laps  = int(clamp(laps, 1, 6))
                tl = r.get("timed_laps")
                if not isinstance(tl, (list, tuple)): 
                    tl = [2]
                # 1..laps 범위로 정리, 비었으면 2 또는 1
                timed = sorted({int(x) for x in tl if 1 <= int(x) <= laps})
                if not timed:
                    timed = [2 if laps >= 2 else 1]
                runs_out.append({"start_sec": start, "laps": laps, "timed_laps": timed})
            except Exception:
                # 이 러닝만 스킵
                continue
        ...
        if not runs_out:
            # 러닝이 없다면 기본 러닝 생성
            rnd = random.Random(hash((name, team, dur)) & 0xffffffff)
            runs_out = _mk_default_runs(rnd, dur)

        return {"name": name, "team": team, "base_vmul": base_v, "runs": runs_out}

    # --------- 1) LLM 시도 ---------
    plans_llm = None
    try:
        from f1sim.ai.llm_client import ask_llm_json  # 외부가 없으면 except로 빠짐
        AI_PLAN_SCHEMA = {
            "name": "quali_ai_plan",
            "schema": {
        ...
        usr_p = (
            ...
            "- Vary base_vmul by driver skill; stronger teams/drivers may push later."
        )
        js = ask_llm_json(AI_PLAN_SCHEMA, sys_p, usr_p, temperature=0.25) or {}
        raw_plans = js.get("plans") or []
        # LLM이 보낸 목록을 others만 남기고 보정
        valid_pairs = {(d.get("name"), d.get("team")) for d in others}
        plans_llm = []
        for p in raw_plans:
            if (p.get("name"), p.get("team")) in valid_pairs:
                coerced = _coerce_one(p, duration_sec)
                if coerced:
                    plans_llm.append(coerced)
    except Exception:
        plans_llm = None  # LLM 경로 실패 → 폴백으로
    ...
    out = []
    for d in others:
        # 드라이버 rating 기반 속도 스케일(없으면 80±5 가정)
        rating = d.get("rating")
        if rating is None:
            rating = 80.0 + rnd.uniform(-5, 5)
        try:
            rating = float(rating)
        except Exception:
            rating = 80.0
        base_v = 0.98 + (clamp(rating, 60.0, 100.0) - 60.0) * (0.12 / 40.0)
        base_v = float(clamp(base_v, 0.96, 1.12))

        runs = _mk_default_runs(rnd, duration_sec)
        out.append({
            "name": _safe_name(d.get("name")),
            "team": _safe_name(d.get("team")),
            "base_vmul": base_v,
            "runs": runs,
        })
    return out
...
def persist_quali_result(session_name: str, payload: dict):
    ...
    save_dir = Path(st.session_state.get("save_dir") or ensure_save_dir())
    out = save_dir / f"quali_{session_name}.json"
    out.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    return str(out)

# Q1→Q2 진출/탈락 확정
def process_q1_and_route(result: dict, player_team: str):
    # best 기준 정렬 (None 은 매우 느린 것으로 간주)
    def key(r): return (999999 if r["best"] is None else r["best"])
    ordered = sorted(result["results"], key=key)[:20]

    top15  = [(r["name"], r["team"]) for r in ordered[:15]]
    bot5   = [(r["name"], r["team"]) for r in ordered[15:20]]

    qs = st.session_state.setdefault("quali_state", {})
    qs["advancers_Q2"] = top15
    qs["q1_p16_p20"]   = bot5

    # 플레이어 팀이 Q2에 1명이라도 가는지 판단
    go_q2 = any(t == player_team for _, t in top15)

    # 다음 페이지 이동
    st.query_params.clear()
    if go_q2:
        try: st.switch_page("pages/05_q2.py")
        except Exception: st.info("Q2 페이지를 찾지 못했습니다. 좌측 사이드바에서 이동하세요.")
    else:
        try: st.switch_page("pages/06_main_race.py")
        except Exception: st.info("메인 레이스 페이지를 찾지 못했습니다.")

# 팀 선택 헬퍼: Q1에서만 선택 허용
def pick_player_team(roster, by_id, *, allow_select: bool):
    teams_in_roster = sorted({r["team"] for r in roster if r.get("team")})

    default_idx = 0
    last = st.session_state.get("player_team_last")
    if last in teams_in_roster:
        default_idx = teams_in_roster.index(last)
    else:
        sel_from_state = st.session_state.get("team_id")
        if sel_from_state:
            nm = by_id.get(str(sel_from_state))
            if nm in teams_in_roster:
                default_idx = teams_in_roster.index(nm)

    if allow_select:
        player_team = st.sidebar.selectbox(
            "플레이어 팀",
            teams_in_roster,
            index=min(default_idx, max(0, len(teams_in_roster)-1)),
        )
    else:
        player_team = last if last in teams_in_roster else (teams_in_roster[0] if teams_in_roster else "")
        st.sidebar.markdown(f"**플레이어 팀:** {player_team or '—'}")

    st.session_state["player_team_last"] = player_team
    return player_team, teams_in_roster

# 런타임 DTO (프론트로 보낼 PLAN)
@dataclass
class DriverPlan:
    name: str
    team: str
...
def run_page():
    ...
    # 쿼리 파라미터로 결과(b64 JSON)가 넘어왔는지 확인
    qp = st.query_params
    if "quali_result_b64" in qp:
        try:
            b64 = qp.get("quali_result_b64")[0] if isinstance(qp.get("quali_result_b64"), list) else qp.get("quali_result_b64")
            js = json.loads(base64.b64decode(b64.encode("ascii")).decode("utf-8"))
            persist_quali_result(SESSION, js)
            # 플레이어 팀 이름은 세션 상태 보존
            player_team = st.session_state.get("player_team_last","")
            process_q1_and_route(js, player_team)
            return
        except Exception as e:
            st.warning(f"결과 저장/전환 실패: {e}")
            st.query_params.clear()
    ...
    if not roster:
        st.error("drivers.csv 필요"); st.stop()

    # Q1: 팀 선택 허용
    player_team, teams_in_roster = pick_player_team(roster, by_id, allow_select=True)

    # PLAN 만들기 (플레이어 2명 + AI 나머지)
    my_drivers = [d for d in roster if d["team"] == player_team]
    if len(my_drivers) == 0 and teams_in_roster:
        fallback = teams_in_roster[0]
        st.warning(f"선택한 팀에 드라이버가 없어 **{fallback}** 로 전환했습니다.")
        player_team = fallback
        st.session_state["player_team_last"] = player_team
        my_drivers = [d for d in roster if d["team"] == player_team]
    my_drivers = sorted(my_drivers, key=lambda x: (x.get("num") or 999))[:2]

    duration_sec = int(DURATION_MIN * 60)
    ai_plans = get_ai_plan(SESSION, circuit, duration_sec, roster, player_team, lap_base)
    ...
    for d in my_drivers:
        ...
        plans.append(DriverPlan(d["name"], d["team"], d["abbr"], d["color"] or "", _img_uri(d["name"]), vm, True, runs=[]))
    for d in roster:
        if d["team"] == player_team: continue
        p = map_ai.get((d["name"], d["team"]))
        vm = float(p["base_vmul"]) if p else (1.0 + rnd.uniform(-0.02, 0.02))
        rr = list(p["runs"]) if p else [{"start_sec": rnd.uniform(10, 140), "laps":3, "timed_laps":[2]},
                                        {"start_sec": rnd.uniform(300, 520), "laps":3, "timed_laps":[2]}]
        plans.append(DriverPlan(d["name"], d["team"], d["abbr"], d["color"] or "", _img_uri(d["name"]), vm, False, runs=rr))
...
<div id="wrap">
  <div id="left">
    ...
    <div id="sessionDone">
      <div class="panel">
        ...
        <div class="muted" style="margin-bottom:10px;">결과를 저장하고 다음 세션으로 넘어가세요.</div>
        <div style="display:flex; gap:8px; justify-content:center;">
          <button id="btnExport" class="btn">결과 JSON 복사</button>
          <button id="btnNext" class="btn primary">다음으로</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div id="right">
    <div class="leader">
      <div style="font-weight:700;margin-bottom:6px;">Leaderboard
...
(function(){
  ...
  document.getElementById('btnNext').addEventListener('click', ()=>{
    ...
    url.searchParams.set('goto', 'AUTO');
  ...
  const cars=[], pendingReleases=[], pendingBoxes=[];
  let selectedIdx=null, sessionBest=null, lastOrder=[]; const prevRank=new Map();
  ...
  window.releaseNow = function(name, team, laps=3, timed=[2]){
    ...
    const lapsFuel = 1 + car.lapsLeft + 1;   // out + hot + in
  ...
  function playerCards(){
    ...
    players.forEach((p)=>{
      ...
      const selTy=document.createElement('select'); ["soft","medium","hard","intermediate","wet"].forEach(k=>{const o=document.createElement('option'); o.value=k;o.textContent=k[0].toUpperCase()+k.slice(1); selTy.appendChild(o);});
      ...
      const bSelect=document.createElement('button'); bSelect.className='btn'; bSelect.textContent='선택';
      const bGo=document.createElement('button'); bGo.className='btn primary'; bGo.textContent='출발(3랩)';
      const bBox=document.createElement('button'); bBox.className='btn'; bBox.textContent='피트인';
      ...
      const apply=()=>{ const car=findCar(p.name,p.team); if(!car) return; car.pace=paceSel.value; car.fuelMix=fuelSel.value; if(car.mode==='pitStopWait') car.nextCompound=selTy.value; tyIcon.src=TIRE_IMGS[selTy.value]||tyIcon.src; };
      selTy.addEventListener('change', apply); paceSel.addEventListener('change', apply); fuelSel.addEventListener('change', apply);
      bSelect.addEventListener('click',()=>{ const car=findCar(p.name,p.team); if(!car) return; const idx=cars.indexOf(car); if(idx>=0) showSelection(idx); });
      bGo.addEventListener('click',()=>{ window.releaseNow(p.name,p.team,3,[2]); });
      bBox.addEventListener('click',()=>{ window.boxNow(p.name,p.team); });
  ...
  function updateTeamTelemetry(){
    PLAN.filter(p=>p.isPlayer).slice(0,2).forEach((p,ix)=>{
      ...
      const tp=document.getElementById(`tm${ix}TirePct`); if(tp) tp.textContent=`${lifePct}%`;
      const tb=document.getElementById(`tm${ix}TireBar`); if(tb) tb.style.width=`${lifePct}%`;
      const fuelKg=Math.max(0,car.fuel).toFixed(1);
      const fk=document.getElementById(`tm${ix}FuelKg`); if(fk) fk.textContent=`${fuelKg}kg`;
      const fb=document.getElementById(`tm${ix}FuelBar`); if(fb) fb.style.width=`${Math.min(100,(car.fuel/(6*FUEL_PER_LAP))*100)}%`;
    });
  }
  ...
  function start(){
    ...
    const pMain=gTrack.querySelector('path:nth-of-type(1)');
    const pPit =gTrack.querySelector('path:nth-of-type(2)');
    gAct.innerHTML=''; cars.length=0;
    for(let i=0;i<PLAN.length;i++){
      const car=mkCar(PLAN[i], i);
      car.sPit = sPitStop;
      const q=ptOn(pPit, car.sPit); placeAt(car.el, car.lab, q.x, q.y);
      cars.push(car); prevRank.set(`${car.name}|${car.team}`, i);
    }
    playerCards(); buildTeamTelemetry();
    // AI 예약 출발
    PLAN.forEach((p,idx)=>{
      if (!p.isPlayer){
        const car=cars[idx]; car._reserv=(Array.isArray(p.runs)? p.runs.map(r=>({t:r.start_sec,laps:r.laps,timed:new Set(r.timed_laps||[2])})):[]);
      }
    });
    setInterval(()=>{
  ...
  function loop(now){
    ...
    if (running){
      ...
      if (simT >= DURATION){
        running=false; sessionDone.style.display='flex';
    ...
    const pMain=gTrack.querySelector('path:nth-of-type(1)');
    const pPit =gTrack.querySelector('path:nth-of-type(2)');
    if(!pMain || !pPit){ requestAnimationFrame(loop); return; }
    ...
    for(const car of cars){
      ...
      if (car.mode==='pit'){
        ...
        const before=car.sPit; car.sPit=(car.sPit + vPit*dtSim)%1;
        const q=ptOn(pPit, car.sPit); placeAt(car.el, car.lab, q.x, q.y);
        ...
        if(w>=1){ car.mode='main'; car.s=sMainOut; car.lap=0; car.lastCross=null; car.inLap=car.wantBox?true:false; car.wantBox=false; }
        ...
        const before=car.s; car.s=(car.s + (1/Math.max(0.1,LAP_BASE))*vmul*dtSim)%1;
    ...
    // Leaderboard by best, stable sort with previous rank
    ...
    lastOrder=sorted; sessionBest = sorted[0] ? (sorted[0].best===null? null : sorted[0].best) : null;
    ...
    renderSelPanel(sessionBest); syncSelRing(); updateTeamTelemetry();
...
<div class="wx">
  ...
  <div class="grid">
    <div class="metric"><b>Air Temp</b><div style="font-size:22px; font-weight:800;">{env['air_temp_c']:.0f}°C</div><div class="note">대기 온도</div></div>
    <div class="metric"><b>Track Temp</b><div style="font-size:22px; font-weight:800;">{env['track_temp_c']:.0f}°C</div><div class="note">트랙 표면 온도</div></div>
    <div class="metric"><b>Rain Probability</b><div class="bar"><div style="width:{env['rain_prob']*100:.0f}%"></div></div><div class="note">{env['rain_prob']*100:.0f}%</div></div>
    <div class="metric"><b>Wetness</b><div class="bar"><div style="width:{env['wetness']*100:.0f}%; background:#22c55e;"></div></div><div class="note">{env['wetness']*100:.0f}%</div></div>
    <div class="metric" style="grid-column: 1/3;"><b>Grip Base</b><div class="bar"><div style="width:{min(100,max(0,int(env['grip_base']*100)))}%; background:#a78bfa;"></div></div><div class="note">{env['grip_base']:.2f}</div></div>
  </div>
