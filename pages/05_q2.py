...
# Q2 전용 독립 실행 페이지
# - Q1에서 올라온 15명만 참가
# - 끝나면 상위 10명은 Q3로, 하위 5명은 본선(11~15 그리드)로 확정
# - 우리팀 드라이버가 Q2에 0명이면 즉시 메인레이스로 이동
# ─────────────────────────────────────────────────────────────────────────────
...
def prompt_weather(circuit:str, session:str):
    ...
    usr_p = (
      ...
      "Respect realistic climate; some circuits rarely see rain."
    )
    return sys_p, usr_p
...
def get_ai_plan(session, circuit, duration_sec, drivers, player_team, lap_base):
    """
    외부 LLM이 있으면 사용하고, 없거나 응답이 이상하면 폴백 플랜 생성.
    """
    def clamp(x, lo, hi):
    ...
    try:
        ...
        usr_p = (
            ...
            "- Vary base_vmul per skill; consider holding pace early laps then push on timed laps."
        )
    ...
    for d in others:
        ...
        t0 = rnd.uniform(10, 100)
        t1 = t0 + rnd.uniform(240, 420)
...
def run_session_q2():
    ...
    # 최신 Streamlit API 사용 (experimental 제거)
    qp = st.query_params
    ...
    adv_q2 = qstate.get("advancers_Q2")

    if not adv_q2:
        # st.session_state["quali"]["Q1"] 결과가 있으면 그걸로 구축
        q1res = (st.session_state.get("quali") or {}).get("Q1")
        if q1res and isinstance(q1res.get("results"), list):
            res = q1res["results"]
            # best가 None이면 큰 수 취급
            res_sorted = sorted(res, key=lambda x: (9999.0 if x.get("best") in (None, '') else float(x.get("best"))))
            adv_q2 = [(r["name"], r["team"]) for r in res_sorted[:15]]
            qstate["advancers_Q2"] = adv_q2
        else:
            # 안전 폴백: 로스터 앞에서 15명
            roster = roster[:15]
            qstate["advancers_Q2"] = [(r["name"], r["team"]) for r in roster]
    else:
        # adv 리스트로 로스터 필터
        allowed = set(adv_q2)
        roster = [r for r in roster if (r["name"], r["team"]) in allowed]

    # 우리팀 드라이버가 0명이라면 즉시 메인레이스로 이동
    my_in_q2 = [r for r in roster if r["team"] == player_team]
    if len(my_in_q2) == 0:
        st.info("우리팀 드라이버가 Q2 진출에 실패했습니다. 메인 레이스로 이동합니다.")
        try:
    ...
    # 내 드라이버(최대 2명)
    my_drivers = sorted([d for d in roster if d["team"] == player_team], key=lambda x: (x.get("num") or 999))[:2]
    ...
    for d in roster:
        ...
        rr = list(p["runs"]) if p else [{"start_sec": rnd.uniform(10, 100), "laps":3, "timed_laps":[2]},
                                        {"start_sec": rnd.uniform(260, 480), "laps":3, "timed_laps":[2]}]
        plans.append(_DP(d["name"], d["team"], d["abbr"], d["color"] or "", _img_uri(d["name"]), vm, False, runs=rr))

    # 화면
    L, R = st.columns([0.66, 0.34])

    # ============ 왼쪽(트랙 + 플레이어 컨트롤 + 종료오버레이) ============
    with L:
        html = r"""
<style>
...
<div id="wrap">
    ...
    <div id="sessionDone">
      <div class="panel">
        ...
        <div style="display:flex; gap:8px; justify-content:center;">
          ...
          <button id="btnNext" class="btn primary">Q3로 넘어가기</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div id="right">
    <div class="leader">
      <div style="font-weight:700;margin-bottom:6px;">Leaderboard (Q2: 15 drivers)
...
(function(){
  ...
  document.getElementById('btnNext').addEventListener('click', ()=>{
    ...
    // Q3로 넘어가는 것은 Streamlit 측에서 처리(세션 상태 기반). 여기선 저장만 함.
    alert('Q2 결과 저장됨. 상단 메뉴에서 Q3 페이지로 이동하세요.');
  });
  ...
  // 페이지가 언마운트되는 순간 Q2 결과를 로컬스토리지에 남겨둔다(안전망)
...
"""
    ...
    # ============ Q2 종료 후: 상태 갱신(세션 사이드) ============
    # - 이 블록은 페이지 하단에서 "다음으로" 버튼 대신 바로 처리하고 싶을 때 참고용입니다.
    #   (지금은 버튼에서 저장만 하고, Q3 페이지에서 st.session_state를 읽습니다.)
